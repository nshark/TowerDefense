...
public class game implements KeyListener, MouseListener, Runnable, MouseMotionListener {
    ...
    public player p;
    public ArrayList<Rnode> Rnodes = new ArrayList<>();
    ...
    public void run() {
        ...
        buttons.add(new button(10,10,100,75,"Back"));
        buttons.add(new button(100,225,900,275,"Research"));
        Rnodes.add(new Rnode(500,250,"playerBase",null,0,
                new String[]{"The starter ship                   ",
                        "Has one cannon."}));
        Rnodes.get(0).r = true;
        Rnodes.get(0).equipped = true;
        ...
        while(true) {
            ...
            if (playing) {
                        ...
                        if (!b2.containsKey(s)) {
                            if (!Objects.equals(s, "turspd") && !Objects.equals(s, "rof")) {
                                b2.put(s, new ShopStatButton(250, (b2.size() % 10) * 50, 750, (b2.size() % 10) * 50 + 50, s,
                                        1, 2, 5, 100, this));
                            } else if (Objects.equals(s, "rof") || Objects.equals(s, "turspd")) {
                                b2.put(s, new ShopStatButton(250, (b2.size() % 10) * 50, 750, (b2.size() % 10) * 50 + 50, s,
                                        2, 0, -1, 100, this));
                            }
                        }
                ...
                if(buttons.get(2).draw(g,mx,my,mp)){
                    mp = false;
                    mainMenu = false;
                    research = true;
                }
        ...
        if(enemies.size() == 0) {
            ...
            while (power < waveCounter) {
                ...
                if (x <= 1) {
                    ...
                    if (p == null) {
                        p = new drone(coords[0], coords[1], 0, 0, true, ran);
                        entities.add(p);
                    } else {
                        p.exist = true;
                        p.x = coords[0];
                ...
                if(x >= 2 && x <= 4){
                    power += 2;
                    Rdrone r = (Rdrone) getUnused(Rdrone.class);
                    int[] coords = this.getXYwall();
                    if(r == null){
                        r = new Rdrone(coords[0], coords[1], 0,0,true);
                        entities.add(r);
                    }
                    else{
                        r.exist = true;
